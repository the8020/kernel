...
- Materialize eval modules, securely copy a local module tree with imports, create sandbox-visible entrypoint URLs, invoke the shared job manager, and return execution/artifact identity plus synchronous cgroup resource data.
...
- Unit tests cover eval wrapper/result delegation, owner-scoped default artifact permissions, synchronous resource snapshots, relative-import copying, path/symlink/control-directory rejection, size limits, detached forwarding, and no host execution.
