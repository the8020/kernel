...
- Identity is a stable `nod-` ID with ten random lowercase alphanumeric
  characters, generated through `identity` and stored in `kernel.toml`. The same
  file stores every persisted per-node kernel setting; it is the only node
  configuration file.
...
- Runtime definitions and materialized images live below `node/kernel/runtime/`;
  the private `database/` directory owns the default single-node SQLite file and
  is never sandbox-mounted.
