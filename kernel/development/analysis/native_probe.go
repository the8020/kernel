...
// A disposable native workload, built by native_test.go, never a runtime asset.
package main
...
func main() {
	...
	if len(os.Args) == 2 && os.Args[1] == "gitdir-file" {
		must(syscall.Rename(".git", ".git-file-probe"))
		must(syscall.Rename(".git-file-probe", ".git"))
	...
	if len(os.Args) == 2 && os.Args[1] == "remove-directories" {
		...
		oldNames, err := held.Readdirnames(-1)
		must(err)
		if len(oldNames) != 0 {
			panic(fmt.Sprintf("removed directory descriptor sees replacement children: %v", oldNames))
		}
		must(os.RemoveAll("private-remove"))
	...
	if len(os.Args) == 2 && os.Args[1] == "remove-shared-directories" {
		...
		must(err)
		if len(names) != 0 {
			panic(fmt.Sprintf("removed shared directory handle sees children: %v", names))
		}
	...
	if len(os.Args) == 2 && os.Args[1] == "rename-lower" {
		// These files are supplied in the shared tree by gofer_probe.py.
