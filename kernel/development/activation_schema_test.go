...
func nativeSchemaJobs(t *testing.T, m *Manager) *jobs.Manager {
	t.Helper()
	...
	sandboxes, err := sandboxmanager.New(sandboxmanager.Config{
		...
		Ports: leases, Logs: logs, History: archive, StartupTimeout: 30 * time.Second, KeepAlive: time.Second, StopGrace: time.Second,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		...
		for {
			...
			case <-t.Context().Done():
				return
			case <-ticker.C:
				if _, err := sandboxes.CleanupIdle(t.Context()); err != nil && t.Context().Err() == nil {
					t.Errorf("schema runtime idle cleanup: %v", err)
				}
			}
		}
	}()
	t.Cleanup(func() { <-maintenanceDone })
	...
	profile := model.RuntimeProfile{
		WorkloadType: model.WorkloadJob, ImageDigest: image.ImageDigest, DependencyMode: model.DependencyOnline,
		...
		}, NetworkMode: "netstack", EgressAllowed: true, ResourceClass: "job:native-test",
	}
	...
	runner, err := jobs.New(groups, workerManager, jobs.Policy{
		...
		Lifecycle: model.LifecyclePolicy{DestroyWhenIdle: true, StopGracePeriod: time.Second}, ExecutionTimeout: 2 * time.Minute, IdleRuntimeTimeout: time.Second, Logger: logs.Logger(),
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNativeActivationSchema(t *testing.T) {
	m, sparse, sandbox, shared := nativeSparseRuntime(t, "schema", 4)
	ctx := t.Context()
	source, err := filepath.Abs("../../..")
	if err != nil {
		t.Fatal(err)
...
`
	...
	nativeExec(t, sparse.RunscDriver, sandbox, prefix+"printf %s "+shellQuote(changed)+" >tables/labels.ts")
...
`
		for name, body := range map[string]string{"programs/" + program + "/program.toml": manifest, "programs/" + program + "/main.ts": code, "hooks/" + phase + ".toml": hook} {
			nativeExec(t, sparse.RunscDriver, sandbox, prefix+"mkdir -p "+shellQuote(filepath.Dir(name))+"; printf %s "+shellQuote(body)+" >"+shellQuote(name))
		}
	}
	...
	nativeExec(t, sparse.RunscDriver, sandbox, "printf alive > /tmp/schema-generation")
	output := nativeExec(t, sparse.RunscDriver, sandbox, "activate --json --message 'Rejected default change'; status=$?; test \"$status\" = 3")
	...
	if got := nativeExec(t, sparse.RunscDriver, sandbox, prefix+"cat tables/labels.ts"); got != changed {
	...
	nativeExec(t, sparse.RunscDriver, sandbox, "set -e; cd "+shellQuote(attempt.Packages[0].Worktree)+"; printf %s "+shellQuote(fixed)+" >tables/labels.ts; git add tables/labels.ts; git -c user.name=Fixture -c user.email=fixture@example.test commit -m 'Keep original default'")
	output = nativeExec(t, sparse.RunscDriver, sandbox, "activate --json --message 'Real schema and hooks'; status=$?; test \"$status\" = 3")
	...
	output = nativeExec(t, sparse.RunscDriver, sandbox, "activate --json --message 'Real schema and hooks'")
	...
	if got := nativeExec(t, sparse.RunscDriver, sandbox, "cat /tmp/schema-generation"); got != "alive" {
		t.Fatalf("schema activation replaced the development runtime: %q", got)
	}
	if got := nativeExec(t, sparse.RunscDriver, sandbox, prefix+"cat tables/labels.ts"); got != fixed {
	...
	indexCalls := 0
	var indexAttempt activationAttempt
	var publishedRevision int64
	nativeExec(t, sparse.RunscDriver, sandbox, prefix+"printf captured >index-refresh.txt")
	for try := 0; try < 2; try++ {
		evaluation, err := evaluator.New(evaluator.Config{Packages: catalog, Jobs: runner, Database: db})
		if err != nil {
			t.Fatal(err)
		...
		output := nativeExec(t, sparse.RunscDriver, sandbox, command)
		var result ActivationResult
		if err := json.Unmarshal([]byte(output), &result); err != nil || result.Success != (try == 1) {
		...
		if try == 0 {
			...
			nativeExec(t, sparse.RunscDriver, sandbox, prefix+"printf later >index-refresh.txt")
		} else {
			var revision int64
			...
			if got := nativeExec(t, sparse.RunscDriver, sandbox, prefix+"cat index-refresh.txt; cat /tmp/schema-generation"); got != "lateralive" {
				t.Fatalf("index retry lost later edits or the live process: %q", got)
			}
		}
	}
	nativeActivationRecovery(t, m, sparse, sandbox, db, func() {
		evaluation, err := evaluator.New(evaluator.Config{Packages: catalog, Jobs: runner, Database: db})
		if err != nil {
			t.Fatal(err)
		}
...
`
	...
	for name, body := range map[string]string{
		...
		nativeExec(t, sparse.RunscDriver, sandbox, "mkdir -p "+shellQuote(path.Dir("/workspace/packages/"+newID+"/"+name))+"; printf %s "+shellQuote(body)+" >"+shellQuote("/workspace/packages/"+newID+"/"+name))
	}
	runLifecycle := func() ActivationResult {
		t.Helper()
		output := nativeExec(t, sparse.RunscDriver, sandbox, "activate --json --message 'Package lifecycle'")
		var result ActivationResult
	...
	nativeExec(t, sparse.RunscDriver, sandbox, "rm -r /workspace/packages/"+newID)
	runLifecycle()
	...
	nativeExec(t, sparse.RunscDriver, sandbox, "mkdir -p /workspace/packages/"+newID+"; printf 'schema = 1\\n' >/workspace/packages/"+newID+"/package.toml")
	runLifecycle()
	if active, err := store.ActivatedPackageCommit(ctx, newID); err != nil || active == "" {
...
func nativeActivationRecovery(t *testing.T, m *Manager, d *workspace, sandbox Sandbox, db *database.Manager, reload func()) {
	t.Helper()
	...
	for _, when := range []string{"before-prepare", "after-prepare", "before", "after"} {
		...
		for id, name := range files {
			nativeExec(t, d.RunscDriver, sandbox, "printf %s "+shellQuote(candidate)+" >"+shellQuote("/workspace/packages/"+id+"/"+name))
		}
		hook := m.schemaDeployment()
		m.SetSchemaDeployment(nativeActivationHook{prepare: func(ctx context.Context, id string, candidates []deployment.Candidate) error {
		...
		run := func(want int) ActivationResult {
			t.Helper()
			output := nativeExec(t, d.RunscDriver, sandbox, fmt.Sprintf("activate --json --message 'Recovery %s'; status=$?; test \"$status\" = %d", when, want))
			var result ActivationResult
			if err := json.Unmarshal([]byte(output), &result); err != nil {
				t.Fatal(err)
			}
		...
		nativeExec(t, d.RunscDriver, sandbox, "printf %s "+shellQuote(later)+" >/workspace/packages/the8020/dev-core/same.txt")
		reload()
		if when == "before" {
		...
		completed := run(0)
		if !completed.Success || len(completed.Packages) != 2 {
		...
		if got := nativeExec(t, d.RunscDriver, sandbox, "cat /workspace/packages/the8020/dev-core/same.txt /tmp/schema-generation"); got != later+"alive" {
