...
- Own deployment cryptographic integrity and the platform JWT transport
  contract.
...
- Authentication JWT uses golang-jwt/v5, EdDSA with Ed25519 only, typ
  `the8020-auth+jwt`, kid equal to the current fingerprint, issuer `the8020`,
  and audience `the8020`. Require iat and exp, exp after iat, unexpired exp, iat
  not in the future, and valid nbf when present, without clock leeway. Tokens
  are at most 8192 bytes.
...
- Tests cover private key persistence/modes/replacement, safe invalid input,
  DB-independent cross-node signatures and routes, strict JWT rejection,
  precedence, and cookie scope. HTTP/Worker and users-package regressions cover
  the policy split.
