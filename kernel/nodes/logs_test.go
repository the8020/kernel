...
func logPeers(t *testing.T, reader LogReader, override ...http.Handler) (*Manager, *Manager, *httptest.Server) {
	...
	server := testPeerServer(t, owner.signing, handler)
	address, portText, err := net.SplitHostPort(strings.TrimPrefix(server.URL, "https://"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLogQueriesUseExactAuthenticatedNodeAndPreserveReferences(t *testing.T) {
	...
	if err == nil {
		response.Body.Close()
		t.Fatal("unauthenticated log query accepted")
	}
...
func TestStalledLogQueryBodyExpiresWithoutReachingTheReader(t *testing.T) {
	...
	_, peer, server := logPeers(t, logReaderFunc(func(context.Context, records.Query) (records.Page, error) {
		calls.Add(1)
		return records.Page{State: "ok"}, nil
	}))
	connection, err := tls.DialWithDialer(&net.Dialer{Timeout: time.Second}, "tcp", strings.TrimPrefix(server.URL, "https://"), peer.signing.ForwardingTLSConfig())
	if err != nil {
		t.Fatal(err)
	}
