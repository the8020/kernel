...
func TestLoopbackManagerAllocatesDistinctPersistentControlPorts(t *testing.T) {
	...
	first, err := manager.Allocate(context.Background(), "sandbox-one", policy)
	if err != nil {
		t.Fatal(err)
	}
	...
	repeated, err := manager.Allocate(context.Background(), "sandbox-one", policy)
	if err != nil || repeated.SupervisorPort != first.SupervisorPort || repeated.InspectorPort != first.InspectorPort {
		t.Fatalf("repeated=%#v err=%v", repeated, err)
	}
	second, err := manager.Allocate(context.Background(), "sandbox-two", policy)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := manager.Check(context.Background(), "sandbox-one"); err != nil {
		t.Fatal(err)
	}
