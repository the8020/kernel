...
mkdir -p "$DENO/kernel" "$DENO/examples" "$DENO/test" "$DENO/worker/testdata" "$DENO/node_modules/unused"
printf '{}\n' > "$DENO/deno.json"
printf '{}\n' > "$DENO/deno.lock"
printf 'export {};\n' > "$DENO/kernel/mod.ts"
touch "$DENO/AGENTS.md" "$DENO/kernel/kernel_test.ts" "$DENO/examples/demo.ts" "$DENO/test/helper.ts"
...
cmp "$DENO/kernel/mod.ts" "$TEST_ROOT/image/kernel/mod.ts"
[[ $(stat -c %a "$TEST_ROOT/image/new-module/nested/codec.ts") == 444 ]]
[[ $(find "$TEST_ROOT/image" -type f | wc -l) == 2 ]]
