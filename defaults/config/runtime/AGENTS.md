...
- Own pinned runtime versions and checksums, generic protocol source, the Deno
  supervisor and Worker bootstrap, generic HTTP/WebSocket transport,
  kernel-capability, and immutable execution-context SDKs, image definitions,
  portable/full materialization, and runtime-specific tests.
...
- `install.sh` refreshes this tracked tree into each instance's
  ...
  reused.
- The image contains only generic runtime sources and OS/runtime dependencies.
  Application libraries load through ordinary package imports and the shared
  native Deno cache; no application dependency preparation or bundling runs in
  image construction or kernel startup.
- Portable construction materializes the pinned OCI base without copying host
  ...
  namespace. The subsequent non-root chroot smoke verifies installed generic
  runtime imports. Full construction uses the same staged generic runtime and
...
- The service image runs non-root and includes pinned Deno, generic runtime
  modules/protocol, and required administrator debugging tools.
  `stage-service-runtime.sh` excludes tests, DOX, examples, and application
  code.
...
- `stage-service-runtime.sh --sources` lists production TypeScript recursively,
  ...
  nested sources participate automatically. Full image construction copies the
  complete staged `runtime/` directory.
- The image import map exposes the activated read-only package tree through the
  ...
  runtime mappings. Use native npm cache resolution without an image-local
  `node_modules` tree or application lockfile writes.
...
- Portable verification launches the staged rootfs as UID/GID 1993 through the
  pinned rootless runsc and imports the generic kernel and context modules
  before publishing image and smoke records. An enclosing Docker build verifies
  the same modules as UID/GID 1993 inside its build sandbox, records that
  narrower provenance, and the container entrypoint replaces it with a real
