...
deno cache --config "$RUNTIME_ROOT/deno.json" \
  --lock "$RUNTIME_ROOT/deno.lock" --frozen npm:kysely@0.29.4 npm:zod@4.1.5
...
rm -f -- "$TEMPORARY"
deno eval --config "$RUNTIME_ROOT/deno.json" --cached-only --check \
  'import { z } from "@the8020/http"; import { z as upstream } from "zod"; const complete: typeof upstream = z; const schema: upstream.ZodString = complete.string().trim(); if (schema.parse(" ready ") !== "ready") throw new Error("bundled Zod contract failed")'
